...
- `tinynav tunnel --serial <serial>` requests a tunnel config from the Uniflex tunnel API, ensures `cloudflared` is installed, runs the returned `install_command`, and saves the JSON response to `${XDG_DATA_HOME:-$HOME/.local/share}/tinynav/tunnel.json`.
