...
- `tinynav tunnel [--serial <serial>]`
  - requests a tunnel config from the Uniflex tunnel API
  - uses the local machine hostname as the default serial when omitted
  - ensures `cloudflared` is installed
  - runs the returned `install_command`
  - saves the JSON response to `${XDG_DATA_HOME:-$HOME/.local/share}/tinynav/tunnel.json`
